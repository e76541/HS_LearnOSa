...
- 回覆分段要清楚(標題或空行分開意群,勿整段糊成一塊)。
- 回覆中使用代號(站位 F1～F4、待裁項 A/B/C 編號、變更列 #、成品代號等)時,末尾用**表格**集中列本輪代號＋一句白話。
...
| 日誌 | 交接時的現場紀錄? | `docs/logs/INDEX.md` |
...
| 名詞 | 作業用語定義? | `docs/reference/terms/INDEX.md` |
| 共同語言 | LEARNOSA 內容製作用語與禁混別名? | `docs/specs/2026-08-04-learnosa-content-glossary.md` |
| 詞彙分流 | 內容／領域／治理三層同名詞怎麼查? | `docs/specs/2026-08-04-learnosa-hs-learnedge-bridge.md` §1 |
...
其餘作業用語完整定義見「名詞」;回覆用詞須與已登記正名一致;禁以「卡」當工作代稱(見規範第七節)。
...
5. **同輪**依規範第五節「日誌報告」寫 `docs/logs/` 並更新「日誌」INDEX;缺則不得宣稱交接完成。
6. 回報完成範圍、檢查結果、未解事項及下一步,附簡易變更表本次區間。

交接不得執行任何 Git 指令。接手預設不讀日誌。
...
3. 比對差異,輸出**升級更新說明**:只列通用層變動;專案層(作業模塊登記表、術語分流、Skill 表、詞彙路徑)不碰;本專案自行改過的通用條文列為衝突,由使用者裁決。降版時標明將移除的規則。
