...
| [specs/2026-07-27-usage-first-learning-loop.md](specs/2026-07-27-usage-first-learning-loop.md) | 草稿 | 無 | [流程圖](specs/assets/2026-07-27-usage-first-loop.html) | **使用優先學習迴路**：六步＋使用卡；**§2.1 規則深度三層**（運用／公式／深度；來源可跳、前台不許跳；難易＝准看最深層）；前台／後台／**索引底層**三角色；下鑽白名單＋防過度；**§6 三類分流**；§7 待裁 a–c（d 已裁一主一副）；未裁不生效，可先試作 |
