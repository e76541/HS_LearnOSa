...
> **狀態**：**已反映**（2026-07-22 落表 M0＋F／T／R／V；2026-07-23 F3 拆四平行小塊、舊 F4 併入 F3c；**2026-07-27 站位重排**：F3a 文本型導航／F3b 專案型導航／F3c 題目、F3d 退場、**F4 復活＝產出站（講圖／演講）**；九宮同日裁定放棄）。  
...
| `({範圍})` | 覆蓋的 F 跨度，如 `(F1-F4)`（F3＝F3a～F3c 三平行小塊） |
...
| **`C2-(F1-F4)`** | 全管線總圖（成品代號） | 含 F3a～F3c 與 F4；全部參與塊入圖，**不**加 `::` 清單。**真相**＝本檔 ASCII／Mermaid；尚無獨立現行 HTML 投影（舊示意已封存） |

標號沿革：`C2-(F1-F4)`（2026-07-22 初版）→ `C2-(F1-F3)`（2026-07-23 舊 F4 併入 F3c）→ **`C2-(F1-F4)`（2026-07-27 F4 復活，範圍恢復）**。子集舉例：`C3-(F1-F2):: F1, T1, F2`。  
...
| **F** | 主管線 | 站位 F1、F2、**F3a～F3c**（平行可互調）、**F4 產出** |
...
| **F3** | （家族名） | F3a～F3c 三平行小塊的合稱 | 屬 **F**；**不是**單一流水線站 |
| **F3a** | 文本型導航 | 起點（架構圖許可可隨機）→ 模塊 → … → Goal（原「策略 V2／決策牌」，決策投影層 2026-07-27 裁定整層退出） | 屬 **F**；平行可互調 |
| **F3b** | 專案型導航 | 起點 → 技能書＋參考書 → Gate → … → Goal（原 F3b 九宮已放棄） | 屬 **F**；平行可互調 |
| **F3c** | 題目練習 | 出題與作答；承接架構圖標「要練習」的出口 | 屬 **F**；曾承接 2026-07-23 前的舊 F4 |
| **F3d** | （已退場） | — | 2026-07-27 演講併入 F4；**不回收號**作別義 |
| **F4** | 產出站（復活） | 講圖 → 演講（PPT）；九宮 2026-07-27 放棄 | 屬 **F**；2026-07-23～27 間曾併入 F3c，該期歷史列不回改 |
| **T2** | 繪圖 | SVG／概念圖等 | 屬 **T**；吃 F2；與 T3 可平行或串行 |
| **T3** | HTML | hub／頁面輸出 | 屬 **T**；**C1** 為此站成品 |
| **V1** | 驗證 | 升格／開版證據；橫切抽查 | 屬 **V** |

**F3a～F3c 互調**：只許明示呼叫（next／入口），不建成唯一總線；各格皆可作當前入口。

**F3 導航（2026-07-27 重排後）**：

```text
架構圖（以模塊＋邊重算，投影不落庫）
  → 建 Gate 與 Goal
  → 比對技能書（Skill book）／參考書（Reference book）
  → 標三種缺：要補技能 ／ 要補參考 ／ 要練習
...
> F3a～F3c 虛線＝可互調，非強制順序。T2／T3 可選串行。F4＝產出站（講圖→演講；九宮已放棄）。
