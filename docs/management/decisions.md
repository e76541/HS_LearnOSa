...
| 2026-07-22 | 作業編號延伸：**M0→C0**；成品 `C{n}-({範圍})`，全量不加 `::`；T1／R1 同表；**T2／T3 可平行或串行**（皆可出 F2）。見 [ops-id-legend.md](ops-id-legend.md)。**未改** agent-ops。 | 對話 | 草稿 |
| 2026-07-22 | **C1 = HTML 管理中心**（`C1-(T3)`）；原全管線主圖順延為 **`C2-(F1-F4)`**。落表前實作／變更列仍記 M2。見 [ops-id-legend.md](ops-id-legend.md)。**未改** agent-ops。 | 對話 | 草稿 |
| 2026-07-22 | **V1 暫空**：啟用條件／必做檢查／登記面列名尚未補，先佔位不補完。 | 對話 | 草稿 |
| 2026-07-22 | **舊 M2 一體草案語境廢止**：正式對齊 INDEX／三份視圖相關檔頭後繼；分拆為 F2／T2／T3／C1-(T3)／INI-005。見 [INDEX](../INDEX.md)「舊 M2 一體語境」。 | 對話 | 已反映 |
| 2026-07-22 | **C1 測試暫取消**：`test-html-hub.py` 整類 skip；M2 必做「300 檔效能」暫停；後續另做。 | 對話 | 已反映 |
